...
async function generatePlaylist(query: spotifyQuery) {
  try {
    ...
    for (let i = 0; i < 6; i++) { // Making 6 API calls to get 300 tracks in total
    ...
    if (allTracks.length > 0) {
      ...
      const sortedTracks = audioFeatures
        ...
        .map((feature) => {
          ...
          return {
            feature,
            differences: {
              valence: calculateFeatureDifference(feature.valence, spotifyFeatures.valence),
              energy: calculateFeatureDifference(feature.energy, spotifyFeatures.energy),
              danceability: calculateFeatureDifference(feature.danceability, spotifyFeatures.danceability),
              tempo: calculateFeatureDifference(feature.tempo, spotifyFeatures.tempo),
              acousticness: calculateFeatureDifference(feature.acousticness,spotifyFeatures.acousticness),
            },
          };
        })
        .filter(
      ...
      if (topTracks.length > 0) {
        console.log(`Returning ${topTracks.length} tracks matching audio feature criteria:`);
        ...
        // **Task**: Integrate the playlist custom array type output from the index.ts script into this one.
        // Use the 'Track' interface to construct a track object and push each to a 'playlist' array similar to index.ts.
        // The 'Track' interface might include properties like title, artist, album, releaseDate, and duration.
        // You can then map 'topTracks' to this 'playlist' array.
      ...
      return topTracks; // Returning the top tracks
    } else {
      console.log(`No tracks found in the genre '${genre}' between ${startYear} and ${endYear}`);
    }
  } catch (error) {
